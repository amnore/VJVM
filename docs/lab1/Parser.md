...
本章对应 JVM 规范内容：4.1 ~ 4.11。在编写代码前，你需要通读一遍这部分内容并理解
class 文件的结构。我们不会完全实现规范中所有的内容，请自行跳过无关部分。
...
我们在框架中给出了测试使用的比较方法和一个样例（样例在项目的 `testdata` 文件夹下，
里面包含了源代码、编译生成的 class 文件和输出），你可以参考其中的内容。如果你发
现测试代码有什么 bug 或不合理的地方，请联系助教。
...
1. 二进制名称（binary name）：二进制名称是将 `${包名}.${类名}` 中的 `.` 替换成
   ...
   层类名}`。二进制名称的完整规则参见
   [JLS-13.1](https://docs.oracle.com/javase/specs/jls/se17/html/jls-13.html#jls-13.1)
   （class 文件中的二进制名称与 Java 语言规范不同之处在于把 '.' 替换成了 '/'）
...
> javac 和 javap 命令用不了？
>
> 这是因为系统的 `PATH` 环境变量中没有这两个命令所在的目录。在执行一个程序时，系
> 统会在 `PATH` 的包含的目录中去查找可执行文件。如何将一个目录添加到 `PATH` 中请
> 自行搜索。

## 框架代码

框架代码包含了一部分构造 `JClass` 的代码，让我们看看都有什么。

首先，`JClassLoader` 在找到类之后会打开类文件并得到一个字节流（`InputStream`）。
为了方便解析，我们把这个流包装成 `DataInputStream` 传给构造函数。

在构造函数中，我们将会一次读取 class 文件的各部分数据并保存在类中。除了以实现的
部分以外，你还需要实现常量池、父类、接口、成员变量和方法的读取。

所有的常量类型都有一个公共的父类：`Constant`。为了保持 `ConstantPool` 构造方法的
整洁，我们将不同类型常量的构造移到了 `Constant.constructFromData` 这个方法中。
（又是一个静态工厂方法）

框架代码已经实现了 `Integer`、`Float` 等常量的构造，你需要实现其他要求的类型。

除了常量池，框架中也给出了属性、方法和成员变量的部分代码（`Attribute`、
`MethodInfo` 和 `FieldInfo`）。Lab 1.2 中我们不会要求你打印属性信息，因此你可以
不对它做修改。方法和成员变量则需要你自行解析。

> 常量的惰性求值
>
> 对于 `NameAndTypeConstant`，框架代码并没有在构造时就计算出它的 `name` 和
> `descriptor` 属性，而是在第一次调用相关方法时才从常量池获取。这是因为这两个属
> 性可能出现在常量池的任何位置，构造 `NameAndTypeConstant` 时它们可能还没有被读
> 取到。所以框架代码把读取它们的工作延迟到了真正需要的时候。这种在需要时才进行计
> 算的方式被称为“惰性求值”（[Lazy
> Evaluation](https://en.wikipedia.org/wiki/Lazy_evaluation)）。
>
> 相应地，你也可以在构造完常量池后添加一段代码对 `name` 和 `type` 统一进行解析。
> 这种在构造时就把全部值计算出来的方式叫做“贪婪求值”。
