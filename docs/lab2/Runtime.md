...
JVM 的运行环境相关内容可在[手册第二
章](https://docs.oracle.com/javase/specs/jvms/se17/html/jvms-2.html)找到。框架代
码中许多的类都与运行环境中的某个数据结构直接对应，因此你需要首先理解这部分内容。
...
测试代码的输入输出都通过 native 方法实现。为了调用这些方法，你需要实现
`invokestatic` 指令。

[JVM 规
范
](https://docs.oracle.com/javase/specs/jvms/se17/html/jvms-6.html#jvms-6.5.invokestatic)
中关于 `invokestatic` 的行为有十分详细的说明。然而，现阶段你只需要实现以下内容：
...
- `b`：设置断点
  ...
  该命令会在指定函数设置断点。设置时需以 `${类名}:${方法名}:${方法描述符}` 的格
  式指定需要设置断点的方法，其中方法描述符可省略。断点会设置在方法的第一条指令。
  比如：
