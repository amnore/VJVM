...
> 此部分在Java虚拟机规范2.5和4.1章节，请先通读后再来看手册

1. Java堆（Heap）：Java堆是JVM中保存对象的一块内存区域，JVM实例化之后的对象**一般**都存储在该区域。同时这些对象是被**自动内存管理系统（automatic storage management system, 也是常说的GC，垃圾回收器）**所管理，在Java中我们不需要也不能够去显式销毁这些对象。

2. 线程数据

   - Java虚拟机栈：栈中每层函数调用形成一个帧（Frame，也可以叫做栈帧）。每个**帧**中存储有以下数据：

     - **局部变量表**（Local Variable）：JVM通过使用局部变量表来完成方法调用时的参数传递。
     - **操作数栈**（Operand Stack）：JVM 是一个基于栈的虚拟机，所有的运算在一个栈中进行。如 iadd 指令：
       从栈顶取出两个 int，相加结果压回栈顶
     - 槽（Slots）：局部变量表和操作数栈以 4B 为一个单位，long 和 double 占两个单位。对`Slot`的理解：
     ...
     - 当前方法所属类的**运行时常量池**（Run-time Constant Pool）的引用

     > Tips：需要注意的是，栈帧是线程私有的，即我们不能在一个栈帧中调用另一个线程的栈帧
     >
     > Tips：如果有不理解的名词，请自行搜索

   - PC：这个跟大家熟悉的程序计数器是一个东西，只不过在JVM中，每个Java虚拟机线程都有自己的PC。任意时刻JVM只会执行一个线程的方法。通常实现为每个帧保存一个 PC，线程的 PC 为栈顶帧的。
