...
public class MULTINEWARRAY extends Instruction {
    ...
    public void fetchAndRun(JThread thread) {
        ...
        val elemClassRef = (ClassRef) frame.getDynLink().getConstant(thread.getPC().getUnsignedShort());
        val dimensions = thread.getPC().getUnsignedByte();
        assert dimensions >= 1;
        assert elemClassRef.getName().lastIndexOf("[") >= dimensions - 1;
        ...
        try {
            elemClassRef.resolve(frame.getJClass());
            arrClasses[dimensions] = elemClassRef.getJClass();
            for (int i = dimensions - 1; i >= 0; --i) {
                val name = arrClasses[i + 1].getThisClass().getName().substring(1);
